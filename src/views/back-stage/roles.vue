    ...
    <el-dialog title="修改用户信息" :visible.sync="editDialogVisible" width="50%" @close="editDialogClosed">
      <el-form :model="userInfoForm" :rules="rules" ref="editFormRef" label-width="100px" class="demo-ruleForm">
        ...
        <el-form-item prop="email">
          ...
          <el-input v-model="userInfoForm.email" class="ipt"></el-input>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
  ...
  methods: {
    ...
    async saveRoleInfo() {
      ...
      const { data: res } = await this.$http.post(
        this.$originUrl + '/admin/rights',
        this.$qs.stringify({ role_power: this.selectedRoleId, power: Cookie.get('user_power'), id: this.userInfo.id })
      );
    ...
    async editUserInfo() {
      ...
      params.append('email', this.userInfoForm.email);
    ...
    async showEditDialog(row) {
      ...
      const { nickname, username, email} = row
      this.userInfoForm.nickname = nickname;
      this.userInfoForm.username = username;
